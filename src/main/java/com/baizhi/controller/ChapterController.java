...
public class ChapterController {
    ...
    public Map<String, Object> selectByPage(Integer page, Integer rows, String albumId) {
        Integer count = chapterMapper.count(albumId);
        Integer total = count % rows == 0 ? count / rows : count / rows + 1;
        List<Chapter> banners = chapterService.selectByPage(page, rows, albumId);
    ...
    public void upload(MultipartFile filePath, String chapterId, String albumId, HttpSession session) throws IOException, TagException, ReadOnlyFileException, CannotReadException, InvalidAudioFrameException {
        ...
        chapter.setFileSize(bg + "MB");
        chapter.setFileLong(m + "分" + s + "秒");
